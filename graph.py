...
def create_graph(data):
    ...
    num_vertices = int(new_data[1])
    # create the ImageGraph
    imag_g = ImageGraph(size_imag)
    # create vertices - vertex info has the format "x,y,color"
    vertices = new_data[2:num_vertices + 2]
    #print(vertices)

    for i, vertices in enumerate(vertices):
        parts = vertices.split(',')
        #print(parts)
        vertex = ColoredVertex(i, int(parts[0]), int(parts[1]), parts[2])
        imag_g.vertices.append(vertex)

    #imag_g.vertices.append(vertices)
