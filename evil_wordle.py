...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        post: None
        """
        for i, let in enumerate(guessed_word):
            if feedback_colors[i] == CORRECT_COLOR:
                self.colors[let] = CORRECT_COLOR
            elif feedback_colors[i] == WRONG_SPOT_COLOR and self.colors[let] != CORRECT_COLOR:
                self.colors[let] = WRONG_SPOT_COLOR
            elif self.colors[let] == NO_COLOR:
                self.colors[let] = NOT_IN_WORD_COLOR
    ...
    def __str__(self):
        ...
        """
        ## make sure there is no space after the keyboard
        final = ''
        count = -1
        temp = ''
        for row in self.rows:
            check = 0
            for char in row:
                x = color_word(self.colors[char], char)
                temp += x
                check += 1
                if char != 'p' and char != 'l' and char != 'm':
                    temp += ' '
            final += temp
            count += 1
            temp = '\n' + ' ' * 3 ** count
    ...
    def __init__(self, feedback_colors, words):
        ...
        for color in feedback_colors:
            if color == NOT_IN_WORD_COLOR:
                self.difficulty += self.COLOR_DIFFICULTY[NOT_IN_WORD_COLOR]
            elif color == WRONG_SPOT_COLOR:
                self.difficulty += self.COLOR_DIFFICULTY[WRONG_SPOT_COLOR]
            else:
                self.difficulty += self.COLOR_DIFFICULTY[CORRECT_COLOR]
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if copy[i] == guessed_word[i]:
            feedback[i] = CORRECT_COLOR
            copy[i] = '-'
    for i in range(NUM_LETTERS):
        if guessed_word[i] in secret_word:
            if guessed_word[i] in copy and feedback[i] != CORRECT_COLOR:
                feedback[i] = WRONG_SPOT_COLOR
                n = copy.index(guessed_word[i])
                copy[n] = '-'
            elif feedback[i] != CORRECT_COLOR:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    pattern = {}
    temp_lst = []
    if len(remaining_secret_words) == 1:
        feedback_colors_w = get_feedback_colors(remaining_secret_words, guessed_word)
        return feedback_colors_w, remaining_secret_words
    for word in remaining_secret_words:
        feedback_colors_w = get_feedback_colors(word, guessed_word)
        if tuple(feedback_colors_w) in pattern:
            pattern[tuple(feedback_colors_w)].append(word)
        else:
            pattern[tuple(feedback_colors_w)] = [word]
    for key, values in pattern.items():
        fams = WordFamily(key, values)
        temp_lst.append(fams)

    sort_lst = fast_sort(temp_lst)
    #print(sort_lst[0])
    top = sort_lst[0]
    feedback_colors = top.feedback_colors

    new_remaining_secret_words = top.words
