...
On my/our honor, Coen Street and Tlali Engrav, this
...
def step_size(s):
    ...
    """
    s_size = STEP_SIZE_CONSTANT - (hash_word(s, STEP_SIZE_CONSTANT) % STEP_SIZE_CONSTANT)
    return s_size
...
def insert_word(s, hash_table):
    ...
    post: Inserts s into hash_table at the correct index; resolves any collisions
          by double hashing.
    """
    # check if the word already in the list
    if find_word(s, hash_table):
        return
    # determine the index
    hashed = hash_word(s, len(hash_table))
    # insert string if there is None
...
def is_reducible(s, hash_table, hash_memo):
    ...
    if find_word(s, hash_memo):
        return True
    if find_word(s, hash_table) is False: # would this be making is go thru for long time...
        return False
...
def main():
    ...
    for word in word_lst:
        ...
        r = is_reducible(word, hash_lst, hash_memo)
        
        if r:
            reducible_words.append(word)

    #print("done with reducible")

    # find the largest reducible words in reducible_words
    maxm = get_longest_words(reducible_words)
    order_maxm = sorted(maxm)
    # print the reducible words in alphabetical order
    # one word per line
    for word in order_maxm:
        print(word)
